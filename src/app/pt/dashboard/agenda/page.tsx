...
export default function AgendaPagePt() {
  const today = todaySP();
  ...
  async function handleDelete(id: string) {
    if (!confirm("Eliminar este compromisso?")) return;
            ...
            <div className="p-6 space-y-4">
                ...
                <label className={`flex items-center gap-2.5 cursor-pointer select-none rounded-xl p-3 border transition
                  ...
                  <div>
                    ...
                    {form.withMeet && !form.endDate && (
                      <p className="text-xs text-amber-600 mt-0.5">⚠ Preenche a hora de fim para gerar o Meet</p>
                    )}
                  </div>
                </label>
              )}
              {!editing && !googleConnected && (
                <div className="flex items-center gap-2 text-xs text-slate-400 bg-slate-50 rounded-xl p-3 border border-slate-200">
                  <span>🎥</span>
                  <span>Para incluíres o Google Meet, <a href="/pt/dashboard/configuracoes" className="text-blue-500 underline">liga a tua conta Google</a></span>
                </div>
              )}
              <div>
...
function ListView({ appointments, today, onEdit, onDone, onDelete, loading }: {
  ...
  if (loading) return <div className="text-center py-16 text-slate-400 text-sm">A carregar...</div>;
  if (grouped.length === 0) return (
    <div className="text-center py-16 bg-white border border-dashed border-slate-200 rounded-2xl">
      <p className="text-5xl mb-4">🗓️</p>
      <p className="text-slate-600 font-medium">Nenhum compromisso agendado</p>
      <p className="text-slate-400 text-sm mt-1">Clica em &quot;Novo&quot; ou envia pelo WhatsApp.</p>
    </div>
  );
