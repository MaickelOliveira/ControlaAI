...
export default function DrivePagePt() {
  ...
  async function handleDeleteFolder(folderId: string, e: React.MouseEvent) {
    e.stopPropagation();
    if (!confirm("Eliminar esta pasta?")) return;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">
            {files.length} ficheiro{files.length !== 1 ? "s" : ""} · {formatSize(totalSize)} usado{totalSize > 0 ? "s" : ""}
          </p>
        </div>
      ...
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 flex items-start gap-3">
        ...
        <div className="text-sm text-amber-800">
          <p className="font-semibold">Envia ficheiros pelo WhatsApp!</p>
          <p className="text-amber-700 mt-0.5">Envia PDF, imagem ou documento diretamente no chat e a IA organiza automaticamente. Para procurar: <em>&quot;Encontra o comprovativo do mecânico&quot;</em></p>
        </div>
      </div>
      ...
      {loading ? (
        <div className="text-center py-12 text-slate-400 text-sm">A carregar...</div>
      ) : (
        <div className="space-y-6">
                  ...
                  {rootFolders.map(folder => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-xs text-slate-400 mt-0.5">{count} ficheiro{count !== 1 ? "s" : ""}</p>
                      </div>
                    );
                  })}
                </div>
          ...
          <div>
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-3">
              {currentFolder ? `Ficheiros em ${currentFolder.name}` : "Todos os ficheiros"}
            </p>
            {currentFolderFiles.length === 0 ? (
              <div className="text-center py-12 bg-white border border-dashed border-slate-200 rounded-xl">
                <p className="text-4xl mb-3">📂</p>
                <p className="text-slate-500 text-sm">
                  {search ? "Nenhum ficheiro encontrado." : "Nenhum ficheiro aqui ainda."}
                </p>
                {!search && (
                  <p className="text-slate-400 text-xs mt-1">Clica em Upload ou envia pelo WhatsApp.</p>
                )}
              </div>
            ) : (
