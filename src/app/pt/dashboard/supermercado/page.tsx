...
const LIST_TEMPLATES = [
  ...
  { key: "carnes", label: "🥩 Carnes", desc: "Bife, frango, salsicha, entrecosto..." },
  { key: "hortifruti", label: "🥬 Frutas e Legumes", desc: "Banana, tomate, cebola, batata..." },
  { key: "laticinios", label: "🥛 Laticínios", desc: "Leite, queijo, iogurte, manteiga..." },
  { key: "padaria", label: "🍞 Padaria", desc: "Pão, pão de forma, bolachas..." },
  { key: "bebidas", label: "🧃 Bebidas", desc: "Água, refrigerante, sumo, cerveja..." },
  { key: "higiene", label: "🧴 Higiene", desc: "Sabonete, champô, papel higiénico..." },
  { key: "limpeza", label: "🧹 Limpeza", desc: "Detergente, sabão, lixívia..." },
];
...
export default function SupermercadoPagePt() {
  ...
  async function addPurchase(e: React.FormEvent) {
    ...
    if (!r.ok) {
      ...
      setPurchaseError(d.error || "Não consegui registar a compra");
      return;
    }
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-400 mt-1">Muda para o modo pessoal na barra lateral para usares o controlo de supermercado.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="space-y-4">
          ...
          {list.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm">
              ...
              <p className="text-sm text-slate-400 mt-1">Adiciona itens ou gera uma lista por categoria</p>
            </div>
          ) : (
            <div className="space-y-4">
      ...
      {/* ── LISTA PARTILHADA COM O WHATSAPP ── */}
      {tab === "whatsapp" && (
        <div className="space-y-4">
          <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 flex flex-col sm:flex-row sm:items-center gap-3">
            <div className="flex-1">
              ...
              <p className="text-sm text-emerald-700 mt-1">Os itens pedidos ao Zelo aparecem aqui e também no separador Lista. A atualização acontece automaticamente.</p>
            </div>
          ...
          {whatsappItems.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm">
              ...
              <p className="text-sm text-slate-400 mt-1">Envia no WhatsApp: “Criar lista supermercado com arroz, leite e pão”.</p>
            </div>
          ) : (
      ...
      {tab === "compras" && (
        <div className="space-y-3">
          {purchases.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm">
              <p className="text-4xl mb-3">🧾</p>
              <p className="font-semibold text-slate-700">Nenhuma compra registada</p>
            </div>
          ) : purchases.map(p => (
            <div key={p.id} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4">
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-xs text-slate-400">{new Date(p.date + "T12:00:00").toLocaleDateString("pt-PT")} · {p.items.length} itens</p>
                </div>
      ...
      {tab === "comparar" && (
        <div className="space-y-3">
          {prices.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm">
              <p className="text-4xl mb-3">💰</p>
              <p className="font-semibold text-slate-700">Regista compras em pelo menos 2 mercados</p>
      ...
      {tab === "gastos" && (
        <div className="space-y-3">
          {spend.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm">
              <p className="text-4xl mb-3">📊</p>
              <p className="font-semibold text-slate-700">Nenhuma compra registada ainda</p>
            </div>
          ) : (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5">
              <h3 className="font-semibold text-slate-800 mb-4">Onde gastas mais</h3>
              <div className="space-y-4">
                {spend.map((s, i) => {
      ...
      {tab === "catalogo" && (
        <div className="space-y-3">
          <p className="text-xs text-slate-400">O Catálogo não é a lista de compras. É a memória dos produtos que já compraste e serve para organizar o histórico e comparar preços entre mercados.</p>
          {products.length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm">
              <p className="text-4xl mb-3">📦</p>
              <p className="font-semibold text-slate-700">Nenhum produto registado ainda</p>
              <p className="text-sm text-slate-400 mt-1">Aparece aqui conforme registas compras</p>
            </div>
          ) : (
      ...
      {showFinish && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            ...
            <p className="text-xs text-slate-400 mb-4">Fecha os {checkedCount} itens marcados como uma compra, lança em Finanças e limpa a lista. Coloca o preço de cada item para entrar certo na comparação de preços.</p>
      ...
      {showAddPurchase && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="font-bold text-slate-900 mb-4">🧾 Registar Compra</h3>
            <form onSubmit={addPurchase} className="space-y-4">
              ...
              <div className="flex gap-3">
                ...
                <button type="submit" disabled={savingPurchase} className="flex-1 bg-amber-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-amber-700 transition disabled:opacity-50">{savingPurchase ? "..." : "Registar"}</button>
              </div>
            </form>
          </div>
        </div>
