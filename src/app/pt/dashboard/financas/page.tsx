...
export default function FinancasPagePt() {
  ...
  function loadCategories() {
    fetch("/api/categories").then(r => r.json()).then(d => {
      ...
      const defaults = { expense: ["Alimentação","Transporte","Habitação","Saúde","Educação","Lazer","Vestuário","Tecnologia","Serviços","Impostos","Funcionários","Marketing","Fornecedores","Outros"], income: ["Salário","Freelance","Vendas","Investimentos","Renda","Serviços","Reembolso","Outros"] };
  ...
  async function handleSaveCategory(e: React.FormEvent) {
    ...
    const data = await res.json();
    if (!res.ok) { setCatError(data.error || "Erro ao guardar"); setCatSaving(false); return; }
    setCatSaving(false);
    setCatName("");
  ...
  async function handleBulkDelete() {
    ...
    if (!window.confirm(`Eliminar ${selectedIds.size} lançamento(s) selecionado(s)? Esta ação não pode ser desfeita.`)) return;
  ...
  // Só os lançamentos que aparecem no ecrã têm checkbox — "selecionar tudo"
  ...
  return (
    <div className="space-y-5">
          ...
          {selectMode && (
            <div className="flex items-center justify-between gap-2 mb-3 px-3 py-2 rounded-xl bg-slate-50 border border-slate-200">
              ...
              <button onClick={handleBulkDelete} disabled={selectedIds.size === 0 || bulkDeleting}
                ...
                {bulkDeleting ? "A eliminar..." : "🗑️ Eliminar selecionados"}
              </button>
            </div>
          )}
          {loading ? <p className="text-slate-400 text-sm">A carregar...</p> :
            (postedFinances.length === 0 && recs.length === 0 && pendingFinances.length === 0) ? (
              <div className="text-center py-16 text-slate-400">
                ...
                <p className="font-medium text-slate-500">Nenhum registo ainda</p>
                <p className="text-xs mt-1">Envia uma mensagem para o bot ou clica em + Adicionar!</p>
              </div>
            ) : (
              <div className="space-y-1 max-h-[480px] overflow-y-auto">
                ...
                {pendingFinances.map(f => {
                  ...
                  const dueFmt = dueDate.toLocaleDateString("pt-PT");
                ...
                {postedFinances.slice(0, 50).map(f => (
                  <div key={f.id} className="group flex items-center justify-between py-2.5 px-3 rounded-xl hover:bg-slate-50 transition">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="text-sm font-medium text-slate-800 truncate">{f.description}</p>
                        <p className="text-xs text-slate-400">{f.category} · {new Date(f.date + "T12:00:00").toLocaleDateString("pt-PT")}</p>
                      </div>
                    </div>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                {([["once", "Único"], ["recurring", "🔁 Recorrente"], ["installment", "💳 A prestações"]] as const).map(([v, l]) => (
              ...
              <input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                required={form.frequency !== "once"}
                placeholder={form.frequency === "once" ? "Descrição (opcional)" : "Descrição (ex: Netflix, Frigorífico)"}
                  ...
                  {form.frequency === "installment" && (
                    <div className="grid grid-cols-2 gap-3">
                      <input type="number" value={form.totalInstallments} onChange={e => setForm(f => ({ ...f, totalInstallments: e.target.value }))} required
                        placeholder="Nº de prestações" className="border border-slate-200 rounded-xl px-3 py-2.5 text-sm outline-none" />
      ...
      {editRec && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            <h3 className="font-bold text-slate-900 mb-1">✏️ Editar {editRec.recurrenceType === "installment" ? "prestação" : "recorrente"}</h3>
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6">
            ...
            <form onSubmit={handleSaveCategory} className="space-y-3">
              ...
              {(catType === "expense" ? customExpense : customIncome).length > 0 && (
                <div>
                  <p className="text-xs text-slate-400 mb-1.5">As tuas categorias:</p>
      ...
      {deleteTarget && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm p-6 text-center">
            ...
            <h3 className="font-bold text-slate-900 mb-1">Eliminar lançamento?</h3>
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-3 pt-1">
                  ...
                  <button type="button" onClick={handleAnalyzeInvoice} disabled={!importFile || importLoading}
                    className="flex-1 bg-amber-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-amber-700 transition disabled:opacity-50">
                    {importLoading ? "A analisar..." : "Analisar fatura"}
                  </button>
                </div>
              </div>
            ) : (
              <div className="space-y-3">
                ...
                <div className="flex gap-3 pt-1">
                  ...
                  <button type="button" onClick={handleConfirmImport} disabled={importSaving || importItems.filter(i => i.selected).length === 0}
                    className="flex-1 bg-amber-600 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-amber-700 transition disabled:opacity-50">
                    {importSaving ? "A importar..." : `Importar ${importItems.filter(i => i.selected).length} lançamento(s)`}
                  </button>
                </div>
              </div>
            )}
