...
export default function ContatosPagePt() {
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-400 mt-1">Muda para o modo pessoal na barra lateral para gerir contactos.</p>
        </div>
      </div>
    );
  }
      ...
      {loading ? <p className="text-slate-400 text-sm">A carregar...</p> :
        visibleContacts.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm">
            <p className="text-4xl mb-3">📇</p>
            <p className="font-semibold text-slate-700">Nenhum contacto guardado</p>
          </div>
        ) : (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-x-auto">
            <table className="w-full min-w-[560px]">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  {["Contacto", "Vínculo", "Telefone/E-mail", "Estado", ""].map(h => (
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="font-bold text-slate-900 mb-4">{editing ? "✏️ Editar Contacto" : "📇 Novo Contacto"}</h3>
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <input value={form.relation} onChange={e => setForm(f => ({ ...f, relation: e.target.value }))}
                placeholder="Vínculo (ex: esposa, médico, amigo)" className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none" />
