...
export default function LembretesPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (recipientType === "self" && !selfPhone) {
      setFormError(linkedPhones.length ? "Selecione qual número deve receber o lembrete." : "Vincule um WhatsApp em Configurações antes de criar o lembrete.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        setFormError(result.error || "Não foi possível salvar o lembrete. Tente novamente.");
        return;
      }
    } catch {
      setFormError("Não foi possível salvar o lembrete. Verifique sua conexão e tente novamente.");
      return;
    } finally {
      setSaving(false);
    }
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="mt-2 space-y-2">
                    {linkedPhones.length === 0 ? (
                      <p className="text-xs text-amber-700 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2.5">
                        Nenhum WhatsApp vinculado. Vincule um número em <strong>Configurações</strong>.
                      </p>
                    ) : (
                      <>
                        <p className="text-xs text-slate-500">Escolha somente um número para receber:</p>
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button type="submit" disabled={saving || (recipientType === "self" && !selfPhone)}
                  ...
                  {saving ? "Salvando..." : editingId ? "Salvar" : "Criar lembrete"}
                </button>
              </div>
            </form>
          </div>
