...
export default function FuncionariosPageEs() {
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="font-semibold text-slate-700">Disponible en Modo Empresa</p>
          <p className="text-sm text-slate-400 mt-1">Cambia al modo empresa en la barra lateral para gestionar empleados.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-red-500 rounded-2xl p-5 text-white shadow-sm">
          <p className="text-xs text-red-100 uppercase tracking-wide">Nómina Mensual</p>
      ...
      {loading ? <p className="text-slate-400 text-sm">Cargando...</p> :
        visibleEmployees.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm">
            <p className="text-4xl mb-3">👥</p>
            <p className="font-semibold text-slate-700">Ningún empleado registrado</p>
          </div>
        ) : (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-x-auto">
            <table className="w-full min-w-[640px]">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  {["Empleado", "Cargo", "Salario", "Pagos recibidos", "Inicio", "Estado", ""].map(h => (
              ...
              <tbody className="divide-y divide-slate-50">
                {visibleEmployees.map(emp => (
                  <tr key={emp.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="px-5 py-4 text-xs text-slate-400">{new Date(emp.startDate + "T12:00:00").toLocaleDateString("es-419")}</td>
                    <td className="px-5 py-4">
                      <span className={clsx("text-xs px-2.5 py-1 rounded-full font-medium border",
                        emp.status === "active" ? "bg-amber-50 text-amber-700 border-amber-200" : "bg-slate-100 text-slate-500 border-slate-200")}>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="font-bold text-slate-900 mb-4">{editing ? "✏️ Editar Empleado" : "👤 Nuevo Empleado"}</h3>
