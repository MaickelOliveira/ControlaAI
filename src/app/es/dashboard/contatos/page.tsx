...
type Contact = { id: string; name: string; phone?: string; email?: string; relation?: string; notes?: string; status: string };

export default function ContatosPageEs() {
  const [mode, setMode] = useState<string>("");
  const [contacts, setContacts] = useState<Contact[]>([]);
  const [loading, setLoading] = useState(true);
  const [showForm, setShowForm] = useState(false);
  const [editing, setEditing] = useState<Contact | null>(null);
  const [form, setForm] = useState({ name: "", phone: "", email: "", relation: "", notes: "" });
  ...
  const visibleContacts = contacts.filter(c => filter === "all" || c.status === filter);

  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (editing) {
      await fetch("/api/admin/contacts", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: editing.id, ...form }) });
    } else {
      await fetch("/api/admin/contacts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
    }
    setShowForm(false); setEditing(null); setForm({ name: "", phone: "", email: "", relation: "", notes: "" }); load();
  }

  function openEdit(c: Contact) {
    setEditing(c);
    setForm({ name: c.name, phone: c.phone || "", email: c.email || "", relation: c.relation || "", notes: c.notes || "" });
    setShowForm(true);
  }

  async function toggleStatus(c: Contact) {
    await fetch("/api/admin/contacts", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: c.id, status: c.status === "active" ? "inactive" : "active" }) });
    load();
  }
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-400 mt-1">Cambia al modo personal en la barra lateral para gestionar contactos.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => { setEditing(null); setForm({ name: "", phone: "", email: "", relation: "", notes: "" }); setShowForm(true); }}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="bg-blue-600 rounded-2xl p-5 text-white shadow-sm">
          <p className="text-xs text-blue-100 uppercase tracking-wide">Activos</p>
          <p className="text-2xl font-bold mt-1">{contacts.filter(c => c.status === "active").length}</p>
        </div>
        <div className="bg-slate-800 rounded-2xl p-5 text-white shadow-sm">
          <p className="text-xs text-slate-300 uppercase tracking-wide">Inactivos</p>
          <p className="text-2xl font-bold mt-1">{contacts.filter(c => c.status === "inactive").length}</p>
        </div>
      </div>
      ...
      {loading ? <p className="text-slate-400 text-sm">Cargando...</p> :
        visibleContacts.length === 0 ? (
          <div className="bg-white rounded-2xl border border-slate-100 p-12 text-center shadow-sm">
            <p className="text-4xl mb-3">📇</p>
            <p className="font-semibold text-slate-700">Ningún contacto guardado</p>
          </div>
        ) : (
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-x-auto">
            <table className="w-full min-w-[560px]">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  {["Contacto", "Vínculo", "Teléfono/Email", "Estado", ""].map(h => (
              ...
              <tbody className="divide-y divide-slate-50">
                {visibleContacts.map(c => (
                  <tr key={c.id} className="hover:bg-slate-50 transition">
                    ...
                    <td className="px-5 py-4 text-sm text-slate-600">{c.relation || "—"}</td>
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
            <h3 className="font-bold text-slate-900 mb-4">{editing ? "✏️ Editar Contacto" : "📇 Nuevo Contacto"}</h3>
            <form onSubmit={handleSubmit} className="space-y-3">
              <input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} required
                placeholder="Nombre completo" className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-200" />
              <input value={form.relation} onChange={e => setForm(f => ({ ...f, relation: e.target.value }))}
                placeholder="Vínculo (ej: esposa, médico, amigo)" className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none" />
              <input value={form.phone} onChange={e => setForm(f => ({ ...f, phone: e.target.value }))}
                placeholder="Teléfono (opcional)" className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none" />
              ...
              <textarea value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))}
                placeholder="Notas" rows={2} className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none resize-none" />
