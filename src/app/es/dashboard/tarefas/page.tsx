...
const STATUS_COLS = [
  { key: "pending", label: "Pendiente", icon: "📌", color: "border-orange-200 bg-orange-50/50" },
  { key: "in_progress", label: "En curso", icon: "🔄", color: "border-blue-200 bg-blue-50/50" },
  { key: "completed", label: "Completada", icon: "✅", color: "border-amber-200 bg-amber-50/50" },
] as const;
...
export default function TarefasPageEs() {
          ...
          return (
            <div key={col.key} className={clsx("bg-white rounded-2xl border-t-4 shadow-sm p-4", col.color)}>
              ...
              <div className="space-y-2.5 min-h-16">
                {loading ? <p className="text-slate-300 text-xs">Cargando...</p> :
                  colTasks.length === 0 ? <p className="text-slate-300 text-xs text-center py-3">Vacío</p> :
                  colTasks.map(t => (
                    <div key={t.id} className="bg-white border border-slate-100 rounded-xl p-3 shadow-sm">
                      ...
                      {t.dueDate && (
                        <p className="text-xs text-slate-400 mb-2">📅 {new Date(t.dueDate + "T12:00:00").toLocaleDateString("es-419")}</p>
                      )}
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6">
            <h3 className="font-bold text-slate-900 mb-4">+ Nueva tarea</h3>
            <form onSubmit={handleAdd} className="space-y-3">
              <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))} required
                placeholder="Título de la tarea" className="w-full border border-slate-200 rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-slate-200" />
              ...
              <div className="flex gap-3 pt-1">
                ...
                <button type="submit" className="flex-1 bg-slate-800 text-white rounded-xl py-2.5 text-sm font-semibold hover:bg-slate-700 transition">Crear</button>
              </div>
            </form>
          </div>
        </div>
