...
export async function GET() {
  ...
  return NextResponse.json(await getFolders(session.sub));
}
...
export async function POST(req: NextRequest) {
  ...
  const folder = await createFolder(session.sub, name.trim(), parentId ?? null);
...
export async function DELETE(req: NextRequest) {
  ...
  const ok = await deleteFolder(session.sub, id);
